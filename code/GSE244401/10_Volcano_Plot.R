# ============================================================================
# Differential Expression Analysis of Transcription Factors
# ============================================================================
# Purpose:
# This script performs differential expression analysis on transcription factors
# using limma/voom framework. It processes four experimental conditions
# Control pre-exercise, Control post-exercise, SCD pre-exercise, SCD post-exercise
# and generates volcano plots for specific TF sets of interest.
#
# Input:
#   1. Four expression matrix files CSV format
#      - expr_CON_T1.csv Control pre-exercise
#      - expr_CON_T2.csv Control post-exercise
#      - expr_SCD_T1.csv SCD pre-exercise
#      - expr_SCD_T2.csv SCD post-exercise
#      Each file has genes in first column, samples in subsequent columns
#   2. TF focus lists specific transcription factors to visualize
#
# Output:
#   1. Differential expression results for all TFs
#   2. Volcano plots showing TF expression changes
#      - Baseline comparison SCD_T1 vs CON_T1
#      - Exercise response in Control CON_T2 vs CON_T1
#      - Exercise response in SCD SCD_T2 vs SCD_T1
#      - Interaction effect DiD analysis
#   3. Focused volcano plots for specific TF sets
#
# Notes:
#   - Gene names are converted to uppercase for consistent matching
#   - Counts data are processed with voom normalization
#   - Non-count data are log2 transformed with +1 pseudocount
#   - FDR correction using Benjamini-Hochberg method
# ============================================================================

# ============================================================================
# Part 1: Full TF Differential Expression Analysis
# ============================================================================

# 0. Load required packages
if (!requireNamespace("BiocManager", quietly=TRUE)) install.packages("BiocManager")
BiocManager::install(c("edgeR","limma"), ask=FALSE, update=FALSE)
install.packages(c("data.table","ggplot2","ggrepel"), dependencies=TRUE)

library(data.table)
library(edgeR)
library(limma)
library(ggplot2)
library(ggrepel)

# 1. Input file paths
f_con_t1 <- "E:/SCD/数据/原始数据/expr_CON_T1.csv"
f_con_t2 <- "E:/SCD/数据/原始数据/expr_CON_T2.csv"
f_scd_t1 <- "E:/SCD/数据/原始数据/expr_SCD_T1.csv"
f_scd_t2 <- "E:/SCD/数据/原始数据/expr_SCD_T2.csv"

tf_file  <- "E:/SCD/数据/构建网络的数据/human_tf_list.txt"

# 2. Read expression matrices first column is gene name, remaining columns are samples
read_expr <- function(path){
  x <- fread(path)
  gene <- x[[1]]
  mat <- as.matrix(x[, -1, with=FALSE])
  rownames(mat) <- make.unique(as.character(gene))
  colnames(mat) <- make.unique(colnames(mat))
  mat
}

con_t1 <- read_expr(f_con_t1)
con_t2 <- read_expr(f_con_t2)
scd_t1 <- read_expr(f_scd_t1)
scd_t2 <- read_expr(f_scd_t2)

genes <- Reduce(intersect, list(rownames(con_t1), rownames(con_t2), rownames(scd_t1), rownames(scd_t2)))
con_t1 <- con_t1[genes, , drop=FALSE]
con_t2 <- con_t2[genes, , drop=FALSE]
scd_t1 <- scd_t1[genes, , drop=FALSE]
scd_t2 <- scd_t2[genes, , drop=FALSE]

expr <- cbind(con_t1, con_t2, scd_t1, scd_t2)

# 2.5 Read TF list and align gene names
tf_list <- fread(tf_file, header=FALSE)$V1
tf_list <- unique(toupper(tf_list))

rownames(expr) <- toupper(rownames(expr))
tf_in_expr <- intersect(tf_list, rownames(expr))

message("TFs in list: ", length(tf_list))
message("TFs found in expr: ", length(tf_in_expr))
if(length(tf_in_expr) < 20){
  message("WARNING: Very few TFs matched. If expression matrix uses ENSG IDs, mapping to symbols is needed.")
}

# 3. limma/voom fitting fit all genes, extract TF results later
meta <- data.frame(
  sample = colnames(expr),
  Group  = c(rep("CON", ncol(con_t1)), rep("CON", ncol(con_t2)),
             rep("SCD", ncol(scd_t1)), rep("SCD", ncol(scd_t2))),
  Time   = c(rep("T1", ncol(con_t1)), rep("T2", ncol(con_t2)),
             rep("T1", ncol(scd_t1)), rep("T2", ncol(scd_t2))),
  stringsAsFactors = FALSE
)
meta$Condition <- factor(paste(meta$Group, meta$Time, sep="_"),
                         levels=c("CON_T1","CON_T2","SCD_T1","SCD_T2"))

is_counts <- all(abs(expr - round(expr)) < 1e-6, na.rm=TRUE) && max(expr, na.rm=TRUE) > 50
message("Detected counts? ", is_counts)

design <- model.matrix(~0 + meta$Condition)
colnames(design) <- levels(meta$Condition)

if(is_counts){
  dge <- DGEList(expr)
  dge <- calcNormFactors(dge)
  v <- voom(dge, design, plot=FALSE)
  fit <- lmFit(v, design)
} else {
  fit <- lmFit(log2(expr + 1), design)
}

contr <- makeContrasts(
  baseline     = SCD_T1 - CON_T1,
  exercise_CON = CON_T2 - CON_T1,
  exercise_SCD = SCD_T2 - SCD_T1,
  interaction  = (SCD_T2 - SCD_T1) - (CON_T2 - CON_T1),
  levels = design
)

fit2 <- eBayes(contrasts.fit(fit, contr))

res_baseline <- topTable(fit2, coef="baseline",     number=Inf, adjust.method="BH", sort.by="P")
res_con_ex   <- topTable(fit2, coef="exercise_CON", number=Inf, adjust.method="BH", sort.by="P")
res_scd_ex   <- topTable(fit2, coef="exercise_SCD", number=Inf, adjust.method="BH", sort.by="P")
res_inter    <- topTable(fit2, coef="interaction",  number=Inf, adjust.method="BH", sort.by="P")

# Keep only TFs
filter_to_tf <- function(tt, tf_set){
  tt$gene <- rownames(tt)
  keep <- toupper(tt$gene) %in% tf_set
  tt <- tt[keep, , drop=FALSE]
  rownames(tt) <- tt$gene
  tt$gene <- NULL
  tt
}
res_baseline_tf <- filter_to_tf(res_baseline, tf_in_expr)
res_con_ex_tf   <- filter_to_tf(res_con_ex,   tf_in_expr)
res_scd_ex_tf   <- filter_to_tf(res_scd_ex,   tf_in_expr)
res_inter_tf    <- filter_to_tf(res_inter,    tf_in_expr)

# 4. Volcano plot function display only, no saving
plot_volcano_tf <- function(tt, title,
                            fdr_cut=0.05, lfc_cut=0.5,
                            label_top=20){
  if(nrow(tt) == 0) stop("No TFs to plot for: ", title)
  
  df <- tt
  df$gene <- rownames(df)
  df$neglog10FDR <- -log10(df$adj.P.Val)
  df$Sig <- "NS"
  df$Sig[df$adj.P.Val < fdr_cut & df$logFC >=  lfc_cut] <- "Up"
  df$Sig[df$adj.P.Val < fdr_cut & df$logFC <= -lfc_cut] <- "Down"
  
  # Label: prioritize significant TFs meeting LFC threshold, supplement with smallest FDR
  lab <- df[df$adj.P.Val < fdr_cut & abs(df$logFC) >= lfc_cut, , drop=FALSE]
  lab <- lab[order(lab$adj.P.Val), , drop=FALSE]
  if(nrow(lab) < label_top){
    extra <- df[order(df$adj.P.Val), , drop=FALSE]
    extra <- extra[!extra$gene %in% lab$gene, , drop=FALSE]
    lab <- rbind(lab, head(extra, label_top - nrow(lab)))
  } else {
    lab <- head(lab, label_top)
  }
  
  ggplot(df, aes(x=logFC, y=neglog10FDR)) +
    geom_point(aes(color=Sig), alpha=0.85, size=2.2) +
    geom_vline(xintercept=c(-lfc_cut, lfc_cut), linetype="dashed") +
    geom_hline(yintercept=-log10(fdr_cut), linetype="dashed") +
    scale_color_manual(values=c(NS="grey70", Up="red3", Down="royalblue3")) +
    labs(title=title, x="log2 Fold Change", y="-log10(FDR)") +
    theme_bw(base_size=12) +
    theme(plot.title = element_text(hjust=0.5)) +
    ggrepel::geom_text_repel(
      data=lab,
      aes(label=gene),
      max.overlaps=Inf,
      size=3
    )
}

# 5. Display four TF volcano plots
p1 <- plot_volcano_tf(res_baseline_tf, "TF Volcano: SCD_T1 vs CON_T1")
print(p1)

p2 <- plot_volcano_tf(res_con_ex_tf, "TF Volcano: CON_T2 vs CON_T1 Exercise response")
print(p2)

p3 <- plot_volcano_tf(res_scd_ex_tf, "TF Volcano: SCD_T2 vs SCD_T1 Exercise response")
print(p3)

p4 <- plot_volcano_tf(res_inter_tf, "TF Volcano: Interaction DiD")
print(p4)

# ============================================================================
# Part 2: Focused TF Volcano Plot Set 1
# ============================================================================

# 2. TF focus list for first set
tf_focus <- c(
  "AHR","ATF4","ATF5","BHLHE40","BHLHE41","CLOCK","CREB1","CREM","DBP",
  "EGR1","EGR3","ESR1","FOXO3","HNF4A","JUND","KLF10","KLF9","NFIL3",
  "NFKB2","NFYA","NPAS2","NCOA1","NR1H3","NR5A1","NR5A2","PPARA","PPARG",
  "PROX1","RELB","RORA","RORC","SP1","SREBF1","STAT5A","STAT5B","TEF",
  "TP53","TWIST1","ZFHX3"
)
tf_focus <- unique(toupper(tf_focus))

# 3. Read expression matrices
con_t1 <- read_expr(f_con_t1)
con_t2 <- read_expr(f_con_t2)
scd_t1 <- read_expr(f_scd_t1)
scd_t2 <- read_expr(f_scd_t2)

genes <- Reduce(intersect, list(rownames(con_t1), rownames(con_t2), rownames(scd_t1), rownames(scd_t2)))
con_t1 <- con_t1[genes, , drop=FALSE]
con_t2 <- con_t2[genes, , drop=FALSE]
scd_t1 <- scd_t1[genes, , drop=FALSE]
scd_t2 <- scd_t2[genes, , drop=FALSE]

expr <- cbind(con_t1, con_t2, scd_t1, scd_t2)

# Standardize case for matching with tf_focus
rownames(expr) <- toupper(rownames(expr))

tf_in_expr <- intersect(tf_focus, rownames(expr))
message("TFs requested: ", length(tf_focus))
message("TFs found in expr: ", length(tf_in_expr))
if(length(tf_in_expr) == 0) stop("No TFs matched. Check if expression matrix rownames are gene symbols not ENSG IDs.")

# 4. limma/voom fitting
meta <- data.frame(
  sample = colnames(expr),
  Condition = factor(c(
    rep("CON_T1", ncol(con_t1)),
    rep("CON_T2", ncol(con_t2)),
    rep("SCD_T1", ncol(scd_t1)),
    rep("SCD_T2", ncol(scd_t2))
  ), levels=c("CON_T1","CON_T2","SCD_T1","SCD_T2"))
)

is_counts <- all(abs(expr - round(expr)) < 1e-6, na.rm=TRUE) && max(expr, na.rm=TRUE) > 50
message("Detected counts? ", is_counts)

design <- model.matrix(~0 + meta$Condition)
colnames(design) <- levels(meta$Condition)

if(is_counts){
  dge <- DGEList(expr)
  dge <- calcNormFactors(dge)
  v <- voom(dge, design, plot=FALSE)
  fit <- lmFit(v, design)
} else {
  fit <- lmFit(log2(expr + 1), design)
}

contr <- makeContrasts(
  baseline = SCD_T1 - CON_T1,
  levels = design
)

fit2 <- eBayes(contrasts.fit(fit, contr))
res_baseline <- topTable(fit2, coef="baseline", number=Inf, adjust.method="BH", sort.by="P")

# Keep only specified TFs
res_baseline$gene <- toupper(rownames(res_baseline))
res_tf <- res_baseline[res_baseline$gene %in% tf_in_expr, , drop=FALSE]
rownames(res_tf) <- res_tf$gene

# 5. Focused volcano plot
plot_volcano_focus <- function(tt, title, fdr_cut=0.05, lfc_cut=0.5){
  df <- tt
  df$gene <- rownames(df)
  df$neglog10FDR <- -log10(df$adj.P.Val)
  
  df$Sig <- "NS"
  df$Sig[df$adj.P.Val < fdr_cut & df$logFC >=  lfc_cut] <- "Up"
  df$Sig[df$adj.P.Val < fdr_cut & df$logFC <= -lfc_cut] <- "Down"
  
  ggplot(df, aes(x=logFC, y=neglog10FDR)) +
    geom_point(aes(color=Sig), alpha=0.9, size=3) +
    geom_vline(xintercept=c(-lfc_cut, lfc_cut), linetype="dashed") +
    geom_hline(yintercept=-log10(fdr_cut), linetype="dashed") +
    scale_color_manual(values=c(NS="grey70", Up="red3", Down="royalblue3")) +
    labs(title=title, x="log2 Fold Change SCD_T1 / CON_T1", y="-log10(FDR)") +
    theme_bw(base_size=12) +
    theme(plot.title = element_text(hjust=0.5)) +
    ggrepel::geom_text_repel(
      aes(label=gene),
      max.overlaps=Inf,
      size=3
    )
}

p_focus1 <- plot_volcano_focus(
  res_tf,
  title = "TF Volcano Focused Set 1: SCD_T1 vs CON_T1",
  fdr_cut = 0.05,
  lfc_cut = 0.5
)
print(p_focus1)

# ============================================================================
# Part 3: Focused TF Volcano Plot Set 2
# ============================================================================

# 2. TF focus list for second set
tf_focus <- c(
  "CEBPA","CREB1","ELK1","EPAS1","FOSL2","GATA6","GLI1","GLI3","FOXA1",
  "HOXA5","HES1","RBPJ","MSX1","MYCN","NFIB","PROX1","RARA","RARG","SIX1",
  "SOX11","SP3","SREBF1","SRF","TBX2","TCF21","THRA","THRB","WT1","HMGA2",
  "MSC","KLF2","SPDEF","LEF1","FOXP2"
)
tf_focus <- unique(toupper(tf_focus))

# 3. Read expression matrices
con_t1 <- read_expr(f_con_t1)
con_t2 <- read_expr(f_con_t2)
scd_t1 <- read_expr(f_scd_t1)
scd_t2 <- read_expr(f_scd_t2)

genes <- Reduce(intersect, list(rownames(con_t1), rownames(con_t2), rownames(scd_t1), rownames(scd_t2)))
con_t1 <- con_t1[genes, , drop=FALSE]
con_t2 <- con_t2[genes, , drop=FALSE]
scd_t1 <- scd_t1[genes, , drop=FALSE]
scd_t2 <- scd_t2[genes, , drop=FALSE]

expr <- cbind(con_t1, con_t2, scd_t1, scd_t2)

# Standardize case for matching with tf_focus
rownames(expr) <- toupper(rownames(expr))

tf_in_expr <- intersect(tf_focus, rownames(expr))
message("TFs requested: ", length(tf_focus))
message("TFs found in expr: ", length(tf_in_expr))
if(length(tf_in_expr) == 0) stop("No TFs matched. Check if expression matrix rownames are gene symbols not ENSG IDs.")

# 4. limma/voom fitting
meta <- data.frame(
  sample = colnames(expr),
  Condition = factor(c(
    rep("CON_T1", ncol(con_t1)),
    rep("CON_T2", ncol(con_t2)),
    rep("SCD_T1", ncol(scd_t1)),
    rep("SCD_T2", ncol(scd_t2))
  ), levels=c("CON_T1","CON_T2","SCD_T1","SCD_T2"))
)

is_counts <- all(abs(expr - round(expr)) < 1e-6, na.rm=TRUE) && max(expr, na.rm=TRUE) > 50
message("Detected counts? ", is_counts)

design <- model.matrix(~0 + meta$Condition)
colnames(design) <- levels(meta$Condition)

if(is_counts){
  dge <- DGEList(expr)
  dge <- calcNormFactors(dge)
  v <- voom(dge, design, plot=FALSE)
  fit <- lmFit(v, design)
} else {
  fit <- lmFit(log2(expr + 1), design)
}

contr <- makeContrasts(
  baseline = SCD_T1 - CON_T1,
  levels = design
)

fit2 <- eBayes(contrasts.fit(fit, contr))
res_baseline <- topTable(fit2, coef="baseline", number=Inf, adjust.method="BH", sort.by="P")

# Keep only specified TFs
res_baseline$gene <- toupper(rownames(res_baseline))
res_tf <- res_baseline[res_baseline$gene %in% tf_in_expr, , drop=FALSE]
rownames(res_tf) <- res_tf$gene

# 5. Focused volcano plot
p_focus2 <- plot_volcano_focus(
  res_tf,
  title = "TF Volcano Focused Set 2: SCD_T1 vs CON_T1",
  fdr_cut = 0.05,
  lfc_cut = 0.5
)
print(p_focus2)